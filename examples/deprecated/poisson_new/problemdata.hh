...
namespace Dune
{
  ...
  struct SinProduct
  {
    typedef FunctionSpace< double, double, dim, 1 >
      FunctionSpaceType;
  ...
  class RotatingAnisotropyProblem 
    : public ProblemInterface< typename U::FunctionSpaceType >
  {
    typedef ProblemInterface< typename U::FunctionSpaceType >
      BaseType;
  ...
  public:
    ...
    typedef typename BaseType::JacobianRangeType
      JacobianRangeType;
    typedef typename BaseType::DomainFieldType
      DomainFieldType;
    ...
    typedef typename BaseType::DiffusionMatrixType
      DiffusionMatrixType;

    typedef array< DiffusionMatrixType, dimDomain > 
      DivergenceDiffunsionMatrixType;
    ...
    void DivK( const DomainType& arg, DivergenceDiffunsionMatrixType& divK ) const
    {
      ...
      divK[0][0][1] = 
        divK[0][1][0] = eps * ( ( -1. * y * ( y * y - x * x ) ) / ( rt * rt ) );
      divK[1][0][0] = eps * ( ( 2. * x * y * y ) / ( rt * rt ) );
      divK[1][1][1] = eps * ( ( -2. * y * x * x ) / ( rt * rt ) );
      divK[1][0][1] = 
        divK[1][1][0] = eps * ( ( -1. * x * ( x * x - y * y ) ) / ( rt * rt ) );
    }

    bool constantK() const { return false; }
  ...
  createProblemFromFunction( const int problemType, const U& u =U() )
  {
    switch( problemType )
    {
      case 0: return new LaplaceProblem< GridImp, U>();
      case 1: return new RotatingAnisotropyProblem< GridImp, U >();
      case 2: return new SphereProblem< GridImp, U>();
  ...
  createProblem()
  {
    ...
    {
      case 0: return createProblemFromFunction< GridImp >( problemType, SinProduct< dimWorld >() );
      case 1: return createProblemFromFunction< GridImp >( problemType, CosProduct< dimWorld >() );
      case 2: return createProblemFromFunction< GridImp >(  problemType, AlbertaSolution< dimWorld >() );
      case 3: return new  LaplaceProblem< GridImp, CornerSolution< dimWorld > >();
