...
channel = IndicatorFunction([([[1.70, 1.75], [0.50, 0.55]], [-1.077632394950]),
                             ...
                             ([[2.65, 2.70], [0.35, 0.40]], [-1.037305900700])], 'channel')
diffusion_factor = {
    'functions': (function_to_grid_function(ConstantFunction(1)) + channel,
...
f = IndicatorFunction([([[0.95, 1.10], [0.30, 0.45]], [2e3]), ([[3.00, 3.15], [0.75, 0.90]], [-1e3]),
                       ([[4.25, 4.40], [0.25, 0.40]], [-1e3])], 'force')
...
def discretize(num_refinements, polorder=1):
    ...
    lhs_op = LincombOperator([
        make_marix_operator(assemble_SWIPDG_matrix(dg_space, diff, diffusion_tensor), 'PRESSURE')
    ...
    dg_product = make_marix_operator(assemble_DG_product_matrix(dg_space, diffusion_factor_bar, diffusion_tensor),
                                     'PRESSURE')
...
t_h_f = FluxVectorSpace.from_data([
    compute_flux_reconstruction(grid, dg_space, rtn_space, diffusion_factor_bar, diffusion_tensor,
                                u_h_f._list[0].impl),
])
...
if grid.num_elements == 16000:
    ...
    t_h = FluxVectorSpace.from_data([
        compute_flux_reconstruction(grid, dg_space, rtn_space, diffusion_factor_max, diffusion_tensor,
                                    u_h._list[0].impl),
    ])
    _, eta_NC, eta_R, eta_DF = compute_estimate(grid, make_discrete_function(dg_space, u_h._list[0].impl, 'u_h'),
                                                make_discrete_function(rtn_space, t_h._list[0].impl,
                                                                       't_h'), f, diffusion_factor_max,
                                                diffusion_factor_max, diffusion_factor_max, diffusion_tensor, 1, 1, 1)
    del u_h
    del t_h
