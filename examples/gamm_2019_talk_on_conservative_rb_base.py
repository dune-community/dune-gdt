...
def simulate_single_greedy_step(fom, dg_product, FluxVectorSpace, rtn_product, t_h_f, compute_flux_reconstruction,
                                compute_estimate, compute_reference_error, max_extensions, num_samples):
    ...
    for ii in range(len(greedy_data['max_err_mus'])):
        ...
        t_h = FluxVectorSpace.from_data([
            compute_flux_reconstruction(mu, u_h._list[0].impl),
        ])
    ...
    for mu in fom.parameter_space.sample_uniformly(num_samples):
        ...
        t_RB = FluxVectorSpace.from_data([
            compute_flux_reconstruction(mu, u_RB._list[0].impl),
        ])
        # project onto Hdiv_0_RB (we know that Hdiv_0_R is orthonormalized wrt rtn_product)
        t_RB_0 = rtn_product.apply2(Hdiv_0_RB, FluxVectorSpace.from_data([
            t_RB._list[0].impl - t_h_f._list[0].impl,
        ]))
