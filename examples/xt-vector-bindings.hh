...
#ifndef DUNE_GDT_EXAMPLES_XT_VECTOR_BINDINGS_HH
#define DUNE_GDT_EXAMPLES_XT_VECTOR_BINDINGS_HH
...
struct VectorExporter
{
  ...
  static void export_(const std::string& classname)
  {
    ...
    // pair<std::vector<double>, std::vector<VectorType>>
    info = boost::python::type_id<std::pair<double, std::vector<Vec>>>();
    reg = boost::python::converter::registry::query(info);
    if (reg == nullptr || (*reg).m_to_python == nullptr)
      std_pair_to_python_converter<std::vector<double>, std::vector<Vec>>();
...
#endif // DUNE_GDT_EXAMPLES_XT_VECTOR_BINDINGS_HH
