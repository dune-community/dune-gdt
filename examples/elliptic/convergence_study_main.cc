    ...
    for (size_t ii = 0; ii <= num_refinements; ++ii) {
      ...
      try {
        ...
        if (ii == 0)
          ...
          const double eoc_value = std::log(l2_errors[ii] / l2_errors[ii - 1])
        ...
        if (ii == 0)
          ...
          const double eoc_value = std::log(h1_errors[ii] / h1_errors[ii - 1])
              / std::log(grid_width[ii] / grid_width[ii - 1]);
          std::stringstream eoc_string;
          eoc_string << std::setw(8) << std::setprecision(2) << std::fixed << eoc_value;
...
//      info << std::endl;
      ...
      const SIPDG_Adhoc_1_DiscretizationType testcase_3_adhoc_sipdg_1_reference_discretization(*reference_grid_part,
                                                          ...
                                                          testcase_3_dirichlet,
                                                          testcase_3_neumann);
      ConvergenceStudy< SIPDG_Adhoc_1_DiscretizationType >::run(*grid,
