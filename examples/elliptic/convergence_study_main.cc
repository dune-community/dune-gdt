...
int main(int argc, char** argv)
{
  try {
    ...
    if (!boost::filesystem::exists(static_id + ".settings")) {
      ...
      ConvergenceStudy< Example::CGDiscretization< GridPartType, 1 > >::run(grid,
                                                                            reference_discretization,
                                                                            num_refinements,
                                                                            exact_solution,
                                                                            "continuous galerkin, polOrder = 1, error against exact solution");
      info << std::endl << std::endl;
      ConvergenceStudy< Example::CGDiscretization< GridPartType, 2 > >::run(grid,
                                                                            reference_discretization,
                                                                            num_refinements,
                                                                            exact_solution,
                                                                            "continuous galerkin, polOrder = 2, error against exact solution");
      info << std::endl << std::endl;
      ConvergenceStudy< Example::CGDiscretization< GridPartType, 3 > >::run(grid,
                                                                            reference_discretization,
                                                                            num_refinements,
                                                                            exact_solution,
