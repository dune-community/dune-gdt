...
int main(int argc, char** argv)
{
  try {
    ...
    // timer.reset();
    typedef Dune::Detailed::Discretizations::ContinuousLagrangeSpace::
        FemWrapper<GridPartType, polOrder, RangeFieldType, dimRange> SpaceType;
    const SpaceType space(gridPart);

    // left hand side
    // * elliptic diffusion operator
    typedef Dune::Detailed::Discretizations::EvaluationElliptic<DomainFieldType, dimDomain, RangeFieldType, dimRange>
        EllipticEvaluationType;
    typedef Dune::Detailed::Discretizations::LocalOperatorCodim0Integral<EllipticEvaluationType, ExpressionFunctionType>
        EllipticOperatorType;
    const EllipticOperatorType diffusionOperator(*diffusion);
    // * right hand side
    //   * L2 force functional
    typedef Dune::Detailed::Discretizations::EvaluationProduct<DomainFieldType, dimDomain, RangeFieldType, dimRange>
    ...
    info << "'... " << std::flush;
    timer.reset();
    typedef Dune::Detailed::Discretizations::DiscreteFunctionDefaultConst<SpaceType, VectorType>
