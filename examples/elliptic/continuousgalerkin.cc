...
void ensureParamFile(const std::string& filename)
{
  ...
  if (!boost::filesystem::exists(filename)) {
    ...
    file << "grid = "
         << "stuff.grid.provider.cube" << std::endl;
    file << "boundaryinfo = "
         << "stuff.grid.boundaryinfo.alldirichlet" << std::endl;
...
int main(int argc, char** argv)
{
  try {
    ...
    const GridProviderType* gridProvider =
        Dune::Stuff::GridProviders<>::create(description.get(id + ".grid", "stuff.grid.provider.cube"), description);
    ...
    const Dune::shared_ptr<const ExpressionFunctionType> force(
        ExpressionFunctionType::create(description.sub("force")));
    ...
    typedef Dune::Detailed::Discretizations::DiscreteFunction::Default<LagrangeSpaceType, VectorType>
