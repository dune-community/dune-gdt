...
#ifndef DUNE_GDT_EXAMPLES_ELLIPTIC_DISCRETIZATION_SIPDG_HH
#define DUNE_GDT_EXAMPLES_ELLIPTIC_DISCRETIZATION_SIPDG_HH
...
class SymmetricInteriorPenaltyDGAdhoc : public Base<GridPartType, polOrder>
{
...
public:
  ...
  SymmetricInteriorPenaltyDGAdhoc(const GridPartType& gp, const BoundaryInfoType& info, const FunctionType& diff,
  ...
  {
    return "sipdg-adhoc." + Dune::Stuff::Common::toString(polOrder);
  }
            ...
            for (const auto& quadrature_point : face_quadrature) {
              ...
              for (size_t ii = 0; ii < test_basis_entity.size(); ++ii) {
                for (size_t jj = 0; jj < ansatz_basis_entity.size(); ++jj) {
                  ...
                  integrals_entity_entity[ii][jj] +=
                      ...
                      * (-0.5 * diffusion_value_entity * (ansatz_base_gradients_entity[jj][0] * unit_outer_normal)
                         * test_base_values_entity[ii]);
                  // symmetry term
                  integrals_entity_entity[ii][jj] += quadrature_weight * integration_element
                                                     * (-0.5 * ansatz_base_values_entity[jj] * diffusion_value_entity
                                                        * (test_base_gradients_entity[ii][0] * unit_outer_normal));
                  // penalty term
                  integrals_entity_entity[ii][jj] +=
                      quadrature_weight * integration_element
                      * (((8.0 * penalty_factor_) / local_face_weight) * ansatz_base_values_entity[jj]
                         * test_base_values_entity[ii]);
                }
              }
              // * entity / neighbour
              for (size_t ii = 0; ii < test_basis_entity.size(); ++ii) {
                for (size_t jj = 0; jj < ansatz_basis_neighbour.size(); ++jj) {
                  ...
                  integrals_entity_neighbour[ii][jj] +=
                      quadrature_weight * integration_element
                      * (-0.5 * diffusion_value_neighbour * (ansatz_base_gradients_neighbour[jj][0] * unit_outer_normal)
                         * test_base_values_entity[ii]);
                  // symmetry term
                  integrals_entity_neighbour[ii][jj] +=
                      quadrature_weight * integration_element
                      * (0.5 * ansatz_base_values_neighbour[jj] * diffusion_value_entity
                         * (test_base_gradients_entity[ii][0] * unit_outer_normal));
                  // penalty term
                  integrals_entity_neighbour[ii][jj] +=
                      quadrature_weight * integration_element
                      * (-1.0 * ((8.0 * penalty_factor_) / local_face_weight) * ansatz_base_values_neighbour[jj]
                         * test_base_values_entity[ii]);
                }
              }
              // * neighbour / entity
              for (size_t ii = 0; ii < test_basis_neighbour.size(); ++ii) {
                for (size_t jj = 0; jj < ansatz_basis_entity.size(); ++jj) {
                  ...
                  integrals_neighbour_entity[ii][jj] +=
                      quadrature_weight * integration_element
                      * (-0.5 * ansatz_base_values_entity[jj] * diffusion_value_neighbour
                         * (test_base_gradients_neighbour[ii][0] * unit_outer_normal));
                  // penalty term
                  integrals_neighbour_entity[ii][jj] +=
                      quadrature_weight * integration_element
                      * (-1.0 * ((8.0 * penalty_factor_) / local_face_weight) * ansatz_base_values_entity[jj]
                         * test_base_values_neighbour[ii]);
                }
              }
              // * neighbour / neighbour
              for (size_t ii = 0; ii < test_basis_neighbour.size(); ++ii) {
                for (size_t jj = 0; jj < ansatz_basis_neighbour.size(); ++jj) {
                  ...
                  integrals_neighbour_neighbour[ii][jj] +=
                      quadrature_weight * integration_element
                      * (0.5 * ansatz_base_values_neighbour[jj] * diffusion_value_neighbour
                         * (test_base_gradients_neighbour[ii][0] * unit_outer_normal));
                  // penalty term
                  integrals_neighbour_neighbour[ii][jj] +=
                      quadrature_weight * integration_element
                      * (((8.0 * penalty_factor_) / local_face_weight) * ansatz_base_values_neighbour[jj]
                         * test_base_values_neighbour[ii]);
                }
              }
            } // do a face quadrature
...
}; // class SymmetricInteriorPenaltyDGAdhoc


template <class GridPartType, int polOrder>
class SymmetricInteriorPenaltyDG : public Base<GridPartType, polOrder>
{
  typedef Base<GridPartType, polOrder> BaseType;
...
public:
  ...
  SymmetricInteriorPenaltyDG(const GridPartType& gp, const BoundaryInfoType& info, const FunctionType& diff,
                             const FunctionType& forc, const FunctionType& dir, const FunctionType& neu)
    : BaseType(gp, info, diff, forc, dir, neu)
    , space_(BaseType::grid_part())
    , beta_(1.0)
  ...
  std::shared_ptr<DiscreteFunctionType> solve() const
  {
    ...
    const DirichletFunctionalType dirichletFunctional(BaseType::diffusion(), BaseType::dirichlet(), beta_);
    ...
    systemAssembler.addLocalAssembler(
        neumannVectorAssembler, typename SystemAssemblerType::AssembleOnNeumann(BaseType::boundary_info()), rhs_vector);
...
#endif // DUNE_GDT_EXAMPLES_ELLIPTIC_DISCRETIZATION_SIPDG_HH
