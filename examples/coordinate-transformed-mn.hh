...
public:
  ...
  void set_parameters(const std::vector<double>& parameters)
  {
    ...
    rhs_operator_ = std::make_shared<RhsOperatorType>(*grid_view_, *fv_space_, *fv_space_, false);
  ...
  void visualize(const VectorType& alpha_vec, const std::string& prefix)
  {
    ...
    const auto rho_visualizer = std::make_unique<XT::Functions::GenericVisualizer<dimRange, 1, double>>(
        1,
        [&](const int /*comp*/, const auto& val) { return basis_functions_->density(analytical_flux_->get_u(val)); });
    auto vtk_writer = XT::Functions::internal::create_vtkwriter(fv_space_->grid_view(), false);
    // visualize rho
    ...
    XT::Functions::internal::write_visualization(*vtk_writer, prefix);
  }
