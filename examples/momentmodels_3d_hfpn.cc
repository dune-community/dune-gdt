...
int main(int argc, char** argv)
{
  ...
  // triangles. At each refinement, each spherical triangles is divided in 4 smaller triangles. E.g., after one
  // refinement, there are 32 triangles. The hatfunctions are associated with the 4^{num_refinements+1}+2 nodes of the
  // triangulation.
  static constexpr size_t num_refinements = 0;

  using MomentBasis = HatFunctionMomentBasis<double, 3, double, num_refinements, 1, 3>;

  // choose test case here
  using TestCaseType = PointSourcePnTestCase<YASP_3D_EQUIDISTANT_OFFSET, MomentBasis, reconstruct>;
  // using TestCaseType = CheckerboardPnTestCase<YASP_3D_EQUIDISTANT_OFFSET, MomentBasis, reconstruct>;
  // using TestCaseType = ShadowPnTestCase<YASP_3D_EQUIDISTANT_OFFSET, MomentBasis, reconstruct>;
