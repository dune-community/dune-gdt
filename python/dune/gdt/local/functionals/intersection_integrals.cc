...
public:
  using type = GDT::LocalIntersectionIntegralFunctional<I, r, rC, RF, F>;
  ...
  static bound_type bind(pybind11::module& m,
                         ...
                         const std::string& class_id = "local_intersection_integral_functional")
  {
    ...
    m.def(
        ...
        [](const typename type::IntegrandType& unary_intersection_integrand, const int over_integrate) {
          return new type(unary_intersection_integrand, over_integrate);
        },
        "unary_intersection_integrand"_a,
        "over_integrate"_a = 0);
...
struct LocalIntersectionIntegralFunctional_for_all_grids
{
  ...
  {
    using Dune::GDT::bindings::LocalIntersectionIntegralFunctional;

    LocalIntersectionIntegralFunctional<G, I>::bind(m);
    if (d > 1) {
      LocalIntersectionIntegralFunctional<G, I, d, 1, F, F>::bind(m);
      LocalIntersectionIntegralFunctional<G, I, d, d, F, F>::bind(m);
    }
    // add your extra dimensions here
    // ...
    LocalIntersectionIntegralFunctional_for_all_grids<Dune::XT::Common::tuple_tail_t<GridTypes>>::bind(m);
  }
};

template <>
struct LocalIntersectionIntegralFunctional_for_all_grids<Dune::XT::Common::tuple_null_type>
{
  static void bind(pybind11::module& /*m*/) {}
};


PYBIND11_MODULE(_local_functionals_intersection_integrals, m)
{
  namespace py = pybind11;
