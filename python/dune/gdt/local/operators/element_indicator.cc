...
public:
  using type = GDT::LocalElementBilinearFormIndicatorOperator<V, GV, s_r>;
  ...
  static bound_type bind(pybind11::module& m,
                         ...
                         const std::string& class_id = "local_element_bilinear_form_indicator_operator")
  {
...
struct LocalElementBilinearFormIndicatorOperator_for_all_grids
{
  ...
  {
    using Dune::GDT::bindings::LocalElementBilinearFormIndicatorOperator;
    using Dune::XT::Grid::bindings::grid_name;

    LocalElementBilinearFormIndicatorOperator<V, GV>::bind(m, vector_id, grid_name<G>::value());
    if (d > 1) {
      LocalElementBilinearFormIndicatorOperator<V, GV, d>::bind(m, vector_id, grid_name<G>::value());
    }
    // add your extra dimensions here
    // ...
    LocalElementBilinearFormIndicatorOperator_for_all_grids<V, Dune::XT::Common::tuple_tail_t<GridTypes>>::bind(
        m, vector_id);
  }
};

template <class V>
struct LocalElementBilinearFormIndicatorOperator_for_all_grids<V, Dune::XT::Common::tuple_null_type>
{
  static void bind(pybind11::module& /*m*/, const std::string& /*vector_id*/) {}
};


PYBIND11_MODULE(_local_operators_element_indicator, m)
{
  namespace py = pybind11;
