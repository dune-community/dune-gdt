...
public:
  using type = GDT::LocalElementOperatorInterface<V, GV, s_r, 1, double, r_r, 1, double>;
  ...
  static bound_type bind(pybind11::module& m,
                         ...
                         const std::string& class_id = "local_element_operator")
  {
...
struct LocalElementOperatorInterface_for_all_grids
{
  ...
  static void bind(pybind11::module& m, const std::string& vector_id)
  {
    ...
    if (d > 1) {
      LocalElementOperatorInterface<V, GV, 1, d>::bind(m, vector_id, grid_name<G>::value());
      LocalElementOperatorInterface<V, GV, d, 1>::bind(m, vector_id, grid_name<G>::value());
      LocalElementOperatorInterface<V, GV, d, d>::bind(m, vector_id, grid_name<G>::value());
    }
    // add your extra dimensions here
    // ...
    LocalElementOperatorInterface_for_all_grids<V, Dune::XT::Common::tuple_tail_t<GridTypes>>::bind(m, vector_id);
  }
};

template <class V>
struct LocalElementOperatorInterface_for_all_grids<V, Dune::XT::Common::tuple_null_type>
{
  static void bind(pybind11::module& /*m*/, const std::string& /*vector_id*/) {}
};


PYBIND11_MODULE(_local_operators_element_interface, m)
{
  namespace py = pybind11;
