...
public:
  using type = GDT::LocalIntersectionNormalComponentProductIntegrand<I, r, F>;
  using base_type = GDT::LocalBinaryIntersectionIntegrandInterface<I, r, 1, F, F, r, 1, F>;
  using bound_type = pybind11::class_<type, base_type>;
  ...
  static bound_type bind(pybind11::module& m,
                         const std::string& layer_id = "",
                         const std::string& grid_id = XT::Grid::bindings::grid_name<G>::value(),
                         const std::string& class_id = "local_intersection_normal_component_product_integrand")
  {
    namespace py = pybind11;
    using namespace pybind11::literals;
    ...
    bound_type c(m, ClassName.c_str(), ClassName.c_str());
    c.def(
        py::init([](const XT::Functions::GridFunctionInterface<E, d, 1, F>& weight,
                    const bool use_inside_bases,
                    const std::string& logging_prefix) { return new type(weight, use_inside_bases, logging_prefix); }),
        "weight"_a,
        "use_inside_bases"_a = true,
    ...
    m.def(
        FactoryName.c_str(),
        [](const XT::Functions::GridFunctionInterface<E, d, 1, F>& weight,
           const bool use_inside_bases,
           const std::string& logging_prefix) { return new type(weight, use_inside_bases, logging_prefix); },
        "weight"_a,
        "use_inside_bases"_a = true,
...
struct LocalIntersectionProductIntegrands_for_all_grids
{
  ...
  static void bind(pybind11::module& m)
  {
    ...
    LocalIntersectionProductIntegrands_for_all_grids<Dune::XT::Common::tuple_tail_t<GridTypes>>::bind(m);
  }
};

template <>
struct LocalIntersectionProductIntegrands_for_all_grids<Dune::XT::Common::tuple_null_type>
{
  static void bind(pybind11::module& /*m*/) {}
};
...
PYBIND11_MODULE(_local_integrands_intersection_product, m)
{
  ...
  LocalIntersectionProductIntegrands_for_all_grids<XT::Grid::bindings::AvailableGridTypes>::bind(m);
}
