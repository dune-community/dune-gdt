...
struct BoundaryL2Product : public BoundaryL2ProductBase<SpaceType>
{
  ...
  virtual RangeFieldType compute(const FunctionType& function) const override final
  {
    ...
    const auto only_first_half = [=](const GridViewType&, const IT& it) {
      return it.boundary() && it.geometry().center()[0] < 0.5;
    };
    const auto only_second_half = [&](const GridViewType&, const IT& it) {
      return it.boundary() && it.geometry().center()[0] >= 0.5;
    };
