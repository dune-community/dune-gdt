...
public:
  ...
  Discretization(const std::shared_ptr< const GridViewType >& gp,
...
public:
  ...
  virtual double compute_on_current_refinement() DS_OVERRIDE DS_FINAL
  {
    ...
    if (current_level_ != last_computed_level_) {
      ...
      // compute solution
      const auto grid_view = test_.level_grid_view(current_level_);
      const DiscretizationType discretization(grid_view, test_.boundary_info(), test_.diffusion(), test_.force(),
  ...
  virtual std::vector< double > expected_results(const std::string type) const
  {
    if (std::is_same< TestCase,
        EllipticTestCase::ESV07< Dune::ALUGrid<2, 2, Dune::simplex, Dune::conforming > > >::value) {
      if (polOrder == 1) {
        if (type.compare("L2") == 0)
        ...
        DUNE_THROW(Dune::NotImplemented, "Please record the expected results for this polOrder!");
               ...
               EllipticTestCase::LocalThermalBlock< Dune::ALUGrid<2, 2, Dune::simplex, Dune::conforming > > >::value) {
      if (polOrder == 1) {
        if (type.compare("L2") == 0)
      ...
      if (polOrder == 1) {
        ...
        DUNE_THROW(Dune::NotImplemented, "Please record the expected results for this polOrder!");
               ...
               EllipticTestCase::MixedBoundaryTypes< Dune::ALUGrid<2, 2, Dune::simplex, Dune::conforming > > >::value) {
      if (polOrder == 1) {
        if (type.compare("L2") == 0)
