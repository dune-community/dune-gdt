...
public:
                         ...
                         const SecondLocalFunctionType& secondLocalFunction) const
  {
    ...
    // some types we will need
    typedef typename FirstLocalFunctionType::DiscreteFunctionSpaceType DiscreteFunctionSpaceType;

    typedef typename DiscreteFunctionSpaceType::GridPartType GridPartType;

    typedef typename FirstLocalFunctionType::EntityType EntityType;
    ...
    for (unsigned int q = 0; q < numberOfQuadraturePoints; ++q) {
      ...
      const RangeFieldType localOperationEvalauted =
          localOperation_.evaluate(firstLocalFunction, secondLocalFunction, x);
