...
{
private:
  typedef FieldVector<unsigned int, int maxRows> RowDofs;

  typedef FieldVector<unsigned int, int maxColumns> ColumnDofs;

  typedef FieldMatrix<FieldType, int maxRow, maxColumn> MatrixType;
...
public:
  ...
  void setLocalMatrix(unsigned int i, unsigned int j, FieldType val)
  {
    matrix_[i][j] = val;
  }

  FieldType localMatrix(unsigned int i, unsigned int j)
  {
    assert(i < maxRows);
    assert(i < maxColumns);
    return matrix_[i][j];
