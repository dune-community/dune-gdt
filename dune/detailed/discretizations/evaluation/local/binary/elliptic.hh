...
/**
  ...
  **/
template <class FunctionSpaceImp, class
          InducingFunctionImp /*= Dune::Stuff::Function::Expression< typename FunctionSpaceImp::DomainFieldType, FunctionSpaceImp::DimDomain, typename FunctionSpaceImp::RangeFieldType, FunctionSpaceImp::DimRange >*/>
class Elliptic
{
public:
  typedef FunctionSpaceImp FunctionSpaceType;

  typedef Elliptic<FunctionSpaceType, InducingFunctionImp> ThisType;
      ...
      for (unsigned int j = 0; j < cols; ++j) {
        for (unsigned int d = 0; d < JacobianRangeType::rows; ++d) {
          const RangeFieldType gradientProduct =
              gradientLocalAnsatzBaseFunctionSet[i][d] * gradientLocalTestBaseFunctionSet[j][d];
