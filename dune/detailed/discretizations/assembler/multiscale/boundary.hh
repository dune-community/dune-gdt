...
public:
  ...
  Boundary(const BoundaryGridPartType& boundaryGridPart, const Dune::shared_ptr<const BoundaryInfoType> boundaryInfo,
  ...
  void assembleMatrices(const DirichletAssemblerType& dirichletAssembler, MatrixBackendType& matrix) const
  {
    // preparations
    ...
    for (typename BoundaryGridPartType::template Codim<0>::IteratorType entityIt =
      ...
      for (IntersectionIteratorType interectionIt = boundaryGridPart_.ibegin(entity);
        ...
        if (boundaryInfo_->dirichlet(intersection)) {
          dirichletAssembler.assembleLocal(intersection, ansatzSpace_, testSpace_, matrix, tmpLocalMatricesContainer);
