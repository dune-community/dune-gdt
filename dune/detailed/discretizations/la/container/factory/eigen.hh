...
public:
  ...
  {
    return std::make_shared<RowMajorSparseMatrixType>(testSpace.map().size(), ansatzSpace.map().size(), pattern);
  } // static ... createRowMajorSparseMatrix(...)
  ...
  static std::shared_ptr<DenseMatrixType> createDenseMatrix(const TestSpaceType& testSpace,
                                                            const AnsatzSpaceType& ansatzSpace)
  {
    return std::make_shared<DenseMatrixType>(testSpace.map().size(), ansatzSpace.map().size());
