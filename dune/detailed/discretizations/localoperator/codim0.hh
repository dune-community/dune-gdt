...
template <class BinaryEvaluationImp, class LocalizableFunctionImp>
class LocalOperatorCodim0Integral
    : public LocalOperatorCodim0Interface<LocalOperatorCodim0IntegralTraits<BinaryEvaluationImp,
...
public:
             ...
             std::vector<Dune::DynamicMatrix<R>>& tmpLocalMatrices) const
  {
    ...
    const size_t quadratureOrder = std::max(int(localFunction.order()), 0) + std::max(int(ansatzBase.order()) - 1, 0)
