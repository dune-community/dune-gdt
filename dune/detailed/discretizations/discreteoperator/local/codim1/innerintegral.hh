                    ...
                    std::vector< LocalMatrixType >& tmpLocalMatrices ) const
  {
    ...
    if(tmpLocalMatrices.size() < numTmpObjectsRequired()) {
      tmpLocalMatrices.resize(
        ...
        LocalMatrixType(
          std::max(localAnsatzBaseFunctionSetEntity.baseFunctionSet().space().map().maxLocalSize(), localAnsatzBaseFunctionSetNeighbor.baseFunctionSet().space().map().maxLocalSize()),
          std::max(localTestBaseFunctionSetEntity.baseFunctionSet().space().map().maxLocalSize(), localTestBaseFunctionSetNeighbor.baseFunctionSet().space().map().maxLocalSize()),
          RangeFieldType(0.0)));
    } // check tmp local matrices
