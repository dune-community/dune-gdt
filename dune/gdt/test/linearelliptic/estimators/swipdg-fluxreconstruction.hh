  ...
  : public Stuff::Grid::Codim0ReturnFunctor< GridViewType, typename SpaceType::RangeFieldType >
{
  ...
  typedef LocalNonconformityESV2007
      < SpaceType, VectorType, DiffusionFactorType, DiffusionTensorType, GridViewType >       ThisType;
...
class LocalResidualESV2007
  : public Stuff::Grid::Codim0ReturnFunctor< GridViewType, typename ForceType::RangeFieldType >
{
  typedef Stuff::Grid::Codim0ReturnFunctor< GridViewType, typename ForceType::RangeFieldType >     BaseType;
  typedef LocalResidualESV2007
      < SpaceType, VectorType, ForceType, DiffusionFactorType, DiffusionTensorType, GridViewType > ThisType;
...
public:
  ...
  virtual void prepare() override final
  {
    ...
    const Operators::DiffusiveFluxReconstruction< GridViewType, DiffusionFactorType, DiffusionTensorType >
      diffusive_flux_reconstruction(grid_view_,
                                    diffusion_factor_,
                                    diffusion_tensor_,
                                    over_integrate_);
...
{
  typedef LocalDiffusiveFluxESV2007
      < SpaceType, VectorType, DiffusionFactorType, DiffusionTensorType, GridViewType >    ThisType;
  typedef Stuff::Grid::Codim0ReturnFunctor
      < typename SpaceType::GridViewType, typename SpaceType::RangeFieldType >             BaseType;
  typedef typename SpaceType::RangeFieldType                                               RangeFieldType;
  typedef ConstDiscreteFunction< SpaceType, VectorType >                                   ConstDiscreteFunctionType;
  typedef Spaces::RT::PdelabBased< GridViewType, 0, RangeFieldType, SpaceType::dimDomain > RTN0SpaceType;
  typedef DiscreteFunction< RTN0SpaceType, VectorType >                                    RTN0DiscreteFunctionType;
  typedef LocalVolumeIntegralOperator< LocalEvaluation::ESV2007::DiffusiveFluxEstimate
      < DiffusionFactorType, RTN0DiscreteFunctionType, DiffusionTensorType > >             LocalOperatorType;
...
public:
  ...
  static ReturnType estimate(const SpaceType& space,
                             ...
                             const size_t over_int = over_integrate)
  {
    return estimate(space.grid_view(), space, vector, diffusion_factor_norm, diffusion_factor_reconstruction,
  ...
  static ReturnType estimate(const GridViewType& grid_view,
                             ...
                             const size_t over_int = over_integrate)
  {
    ThisType estimator(grid_view, space, vector, diffusion_factor_norm, diffusion_factor_reconstruction,
                       diffusion_tensor, over_int);
    Stuff::Grid::Walker< GridViewType > grid_walker(grid_view);
    grid_walker.add(estimator);
    grid_walker.walk();
...
public:
  ...
  static ReturnType estimate(const SpaceType& space,
                             ...
                             const size_t over_int = over_integrate)
  {
    return estimate(space.grid_view(), space, vector, force, diffusion_factor_norm, diffusion_factor_reconstruction,
                    diffusion_tensor, over_int);
  }
  ...
  static ReturnType estimate(const GridViewType& grid_view,
                             ...
                             const size_t over_int = over_integrate)
  {
    return estimate(grid_view, space, vector, force, diffusion_factor, diffusion_factor, diffusion_tensor,
                    over_int);
  }
  ...
  static ReturnType estimate(const GridViewType& grid_view,
                             ...
                             const size_t over_int = over_integrate)
  {
    ThisType estimator(grid_view, space, vector, force, diffusion_factor_norm, diffusion_factor_reconstruction,
                       diffusion_tensor, over_int);
    Stuff::Grid::Walker< GridViewType > grid_walker(grid_view);
    grid_walker.add(estimator);
    grid_walker.walk();
  ...
  static Stuff::LA::CommonDenseVector< ReturnType > estimate_local(const GridViewType& grid_view,
                                                                   const SpaceType& space,
                                                                   const VectorType& vector,
                                                                   const ForceType& force,
                                                                   const DiffusionFactorType& diffusion_factor_norm,
                                                                   ...
                                                                   const size_t over_int = over_integrate)
  {
    Stuff::LA::CommonDenseVector< ReturnType >
  ...
  virtual ReturnType compute_locally(const EntityType& entity) override
  {
    return eta_nc_.compute_locally(entity)
         + std::pow(  std::sqrt(eta_r_.compute_locally(entity))
                    + std::sqrt(eta_df_.compute_locally(entity)), 2);
  }
...
{
  typedef ESV2007AlternativeSummation
      < SpaceType, VectorType, ForceType, DiffusionFactorType, DiffusionTensorType, GridViewType >            ThisType;
