...
public:
  ...
  {
    ConfigType config = BaseType::default_config(grid_config, true, psi_vac);
    config.add(grid_config, "grid", true);
    config.add(default_boundary_info_config(), "boundary_info", true);
    ConfigType initial_value_config;
    create_initial_value_config(initial_value_config, grid_config);
...
protected:
  ...
  {
    std::cout << "Boundary values have been called !!" << std::endl;
...
public:
  ...
  static ConfigType default_config(const ConfigType grid_config = default_grid_config(),
                                   ...
                                   const RangeFieldImp psi_vac = 5e-9)
  {
    ConfigType config = BaseType::default_config(grid_config, v_points, psi_vac);
    config.add(grid_config, "grid", true);
    config.add(default_boundary_info_config(), "boundary_info", true);
    ConfigType initial_value_config;
    create_initial_value_config(initial_value_config, grid_config, psi_vac, v_points);
    config.add(initial_value_config, "initial_values", true);
    ConfigType rhs_config;
    rhs_config["lower_left"] = grid_config["lower_left"];
    rhs_config["upper_right"] = grid_config["upper_right"];
...
/** \see class TwoBeams in twobeams.hh */
template <class EntityImp, class DomainFieldImp, size_t domainDim, class RangeFieldImp, size_t num_points>
class PlaneSourcePnFirstOrderDG
    : public SourceBeamPnFirstOrderDG<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, num_points>
{
  typedef PlaneSourcePnFirstOrderDG<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, num_points> ThisType;
  typedef SourceBeamPnFirstOrderDG<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, num_points> BaseType;
...
public:
  ...
  using typename BaseType::MatrixType;
  using typename BaseType::RangeType;
  using typename BaseType::PointsVectorType;

  static std::string static_id()
  {
    return BaseType::static_id() + ".planesource_Pn_firstorderdg";
  }

  std::string type() const override
  {
    return BaseType::type() + ".planesource_Pn_firstorderdg";
  }
  ...
  PlaneSourcePnFirstOrderDG(const std::shared_ptr<const FluxType> flux_in,
                            const std::shared_ptr<const RHSType> rhs_in,
                            const std::shared_ptr<const InitialValueType> initial_values_in,
                            const ConfigType& grid_config_in,
                            const ConfigType& boundary_info_in,
...
protected:
  using BaseType::basisfunctions_integrated;
  using BaseType::mass_matrix;
  using BaseType::tridiagonal_matrix_inverse;
  ...
  static void create_initial_value_config(ConfigType& initial_value_config,
                                          ...
                                          const PointsVectorType& v_points = create_equidistant_points())
  {
    const auto basis_integrated = basisfunctions_integrated(v_points);
    initial_value_config["lower_left"] = grid_config["lower_left"];
    initial_value_config["upper_right"] = grid_config["upper_right"];
    initial_value_config["num_elements"] = grid_config["num_elements"];
    initial_value_config["variable"] = "x";
  ...
  // so n-th component of boundary value has to be \psi_{vac}*base_integrated_n at both boundaries.
  // Modell with constant function.
  static std::string create_boundary_values(const PointsVectorType& v_points, const RangeFieldImp psi_vac = 5e-9)
  {
    const RangeType integrated_basis = basisfunctions_integrated(v_points);
    std::string str = "[";
    for (size_t rr = 0; rr < dimRange; ++rr) {
