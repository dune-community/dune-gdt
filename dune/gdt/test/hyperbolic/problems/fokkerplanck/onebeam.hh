      ...
      if (exact_legendre()) {
        ...
        for (size_t rr = 0; rr < dimRange; ++rr) {
          if (rr > 0)
            str += " ";
          if (rr == 0)
            str += DSC::to_string(0.0002 - get_left_boundary_value(rr)) + "*x[0]+" + DSC::to_string(get_left_boundary_value(rr));
          else
            str += DSC::to_string(0.0 - get_left_boundary_value(rr)) + "*x[0]+" + DSC::to_string(get_left_boundary_value(rr));
        }
        str += "]";
        return str;
      } else {
        ...
        for (size_t rr = 0; rr < dimRange; ++rr) {
          if (rr > 0)
            str += " ";
          str += DSC::to_string(0.0001*base_integrated()[rr] - onebeam_left_boundary_values()[rr]) + "*x[0]+" + DSC::to_string(onebeam_left_boundary_values()[rr]);
        }
        str += "]";
        return str;
      }
