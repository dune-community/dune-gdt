    ...
    static std::string create_boundary_values()
    {
      if (exact_legendre()) {
        ...
        for (size_t rr = 0; rr < dimRange; ++rr) {
          if (rr > 0)
            str += " ";
          str += "50*(" + DSC::toString(basefunctions_values_at_minusone()[rr] - basefunctions_values_at_plusone()[rr])
                 + "*x[0]/7.0+" + DSC::toString(basefunctions_values_at_plusone()[rr]) + ")*exp((-(t-1)^2)/2)";
        }
        str += "]";
        return str;
      }
