...
template <class BasisfunctionImp, class GridLayerImp, class U_>
class PlaneSourcePnCharacteristic : public KineticTransportEquationCharacteristic<BasisfunctionImp, GridLayerImp, U_, 1>
{
  typedef KineticTransportEquationCharacteristic<BasisfunctionImp, GridLayerImp, U_, 1> BaseType;
...
public:
  ...
  PlaneSourcePnCharacteristic(const BasisfunctionType& basis_functions,
                              const GridLayerType& grid_layer,
                              const QuadratureType& quadrature = default_quadrature(),
                              const XT::Common::Configuration& grid_cfg = default_grid_cfg(),
                              const XT::Common::Configuration& boundary_cfg = default_boundary_cfg())
  ...
  virtual InitialValueType* create_initial_values() const override
  {
    const DomainType lower_left = XT::Common::from_string<DomainType>(grid_cfg_["lower_left"]);
    const DomainType upper_right = XT::Common::from_string<DomainType>(grid_cfg_["upper_right"]);
    std::cout << grid_layer_.overlapSize(0) << std::endl;
    size_t num_elements = grid_layer_.size(0) - grid_layer_.overlapSize(0);
    num_elements = grid_layer_.comm().sum(num_elements);
    if (num_elements % 2)
      DUNE_THROW(Dune::NotImplemented, "An even number of grid cells is needed for this test!");
    const RangeFieldType len_domain = upper_right[0] - lower_left[0];
    ...
    // approximate delta function by constant value of 1/(2*vol_entity) on cells on both side of 0.
    std::vector<typename ActualInitialValueType::LocalizableFunctionType> initial_vals;
    auto ret_nonchar = basis_integrated * psi_vac_;
    auto ret_delta_nonchar = basis_integrated * (psi_vac_ + 1. / (2. * vol_entity));
    RangeType ret, ret_delta;
    eigenvectors_inverse_.mv(ret_nonchar, ret);
    eigenvectors_inverse_.mv(ret_delta_nonchar, ret_delta);
...
protected:
  ...
  using BaseType::num_segments_;
  using BaseType::psi_vac_;
  using BaseType::eigenvectors_inverse_;
}; // class PlaneSourcePnCharacteristic<...>
