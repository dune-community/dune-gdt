...
public:
  ...
  ShockTube(const std::shared_ptr<const FluxType> flux =
                std::make_shared<DefaultFluxType>(*DefaultFluxType::create(default_config().sub("flux"))),
            const std::shared_ptr<const RHSType> source =
                std::make_shared<DefaultRHSType>(*DefaultRHSType::create(default_config().sub("source"))),
            const std::shared_ptr<const InitialValueType> initial_values = std::make_shared<DefaultInitialValueType>(
                *DefaultInitialValueType::create(default_config().sub("initial_values"))),
            const ConfigType& grid_config                                  = default_grid_config(),
            const ConfigType& boundary_info                                = default_boundary_info_config(),
            const std::shared_ptr<const BoundaryValueType> boundary_values = std::make_shared<DefaultBoundaryValueType>(
                *DefaultBoundaryValueType::create(default_config().sub("boundary_values"))))
    : BaseType(flux, source, initial_values, grid_config, boundary_info, boundary_values)
  {
  }
