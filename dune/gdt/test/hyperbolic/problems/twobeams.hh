      ...
      if (exact_legendre()) {
        ...
        for (size_t rr = 0; rr < dimRange; ++rr) {
          ...
          str += DSC::toString(50 * (basefunctions_values_at_minusone()[rr] - basefunctions_values_at_plusone()[rr]),
                               precision)
                 + "*x[0]+" + DSC::toString(50 * basefunctions_values_at_plusone()[rr], precision);
        }
        str += "]";
        return str;
      }
    ...
    static void calculate()
    {
      if (!is_calculated_) {
        ...
        for (size_t ii = 0; ii < dimRange; ++ii) {
          ...
          basefunction_values[ii] =
              DSC::tokenize(line, ",", boost::algorithm::token_compress_mode_type::token_compress_on);
        }
        ...
        for (size_t ii = 0; ii < dimRange; ++ii) {
          ...
          basefunctions_.emplace_back(
              CGFunctionType(cg_space, basefunction_ii_values, "Basefunction " + DSC::toString(ii)));
        }
        ...
        for (auto it = velocity_grid_view_->template begin<0>(); it != it_end; ++it) {
          ...
          for (size_t ii = 0; ii < dimRange; ++ii) {
            ...
            basefunction_jacobians_values[ii][velocity_grid_view_->indexSet().index(entity)] =
                (basefunctions_[ii].local_function(entity)->jacobian(
                    entity.geometry().local(entity.geometry().center())))[0][0];
          }
        }
        ...
        for (size_t ii = 0; ii < dimRange; ++ii) {
          ...
          for (size_t jj = 0; jj < dimRange; ++jj) {
            ...
            const auto v_times_base =
                DS::Functions::Product<VelocityFunctionType, CGFunctionType>(v, basefunctions_[jj]);
...
public:
  ...
  TwoBeams(const std::shared_ptr<const FluxType> flux_in, const std::shared_ptr<const RHSType> source_in,
