 ...
 * */
template <class Traits, class E, class D, size_t d, class R, size_t r, size_t rC>
class Default : public ProblemInterface<E, D, d, R, r, rC>
{
  typedef Default<Traits, E, D, d, R, r, rC> ThisType;
...
public:
  typedef typename Traits::derived_type ProblemType;
  ...
  static std::unique_ptr<ThisType> create(const XT::Common::Configuration& config = ProblemType::default_config())
  {
    const std::shared_ptr<const typename ProblemType::DefaultFluxType> flux(
        ProblemType::DefaultFluxType::create(config.sub("flux")));
    const std::shared_ptr<const typename ProblemType::DefaultRHSType> rhs(
        ProblemType::DefaultRHSType::create(config.sub("rhs")));
    const std::shared_ptr<const typename ProblemType::DefaultInitialValueType> initial_values(
        ProblemType::DefaultInitialValueType::create(config.sub("initial_values")));
    const ConfigType grid_config = config.sub("grid");
    const ConfigType boundary_info = config.sub("boundary_info");
    const std::shared_ptr<const typename ProblemType::DefaultBoundaryValueType> boundary_values(
        ProblemType::DefaultBoundaryValueType::create(config.sub("boundary_values")));
  ...
  Default(const std::shared_ptr<const FluxType> flux_ptr,
          ...
          const XT::Common::Configuration& grid_cfg,
          const XT::Common::Configuration& boundary_info_cfg,
  ...
  virtual const XT::Common::Configuration grid_config() const override
  {
    return grid_config_;
  }

  virtual const XT::Common::Configuration boundary_info() const override
  {
...
private:
  ...
  const XT::Common::Configuration boundary_info_;
