...
struct MomentApproximation
{
  ...
  // fullmoments, we fix the number of intervals. For the hatfunctions and partial moments we subdivide the intervals
  // for the quadrature such that there are at least as many intervals as for the full moments. Thus we have a
  // different quadrature for each model, but the high order of the quadratures should ensure that the quadrature error
  // is negligible compared to the model error.
  ...
  // of the initial octants, so we can use the same quadrature (on the finest triangulation) for all models.
  ...
  {
    static int quad_refinements_or_intervals(const int num_quad_refinements)
    {
      return num_quad_refinements;
    }
  ...
  {
    static int quad_refinements_or_intervals(const int num_quad_intervals)
    {
      return MomentBasisType::num_intervals == size_t(-1) ? num_quad_intervals : MomentBasisType::num_intervals;
    }
  ...
  // In 1d, min_quad_refinements_or_intervals is a lower bound on the number of quadrature intervals, in 3d it is a
  // lower bound on the number of refinements of the initial spherical triangles
  static void run(const int min_quad_refinements_or_intervals, std::string testcase, std::string filename = "")
  {
    ...
    const int quad_refinements_or_intervals =
        QuadratureHelper<>::quad_refinements_or_intervals(min_quad_refinements_or_intervals);
    ...
    } else if (testcase == "CrossingBeams1d") {
      if (dimDomain != 1)
        DUNE_THROW(InvalidStateException,
                   "This is a 1-dimensional test, but the basis functions are " + XT::Common::to_string(dimDomain)
                       + "-dimensional!");
    ...
    const auto visualization_quadratures =
        QuadratureHelper<>::get(*triangulation, quad_refinements_or_intervals, true, additional_quad_refinements);
    const auto basis_quadratures =
        QuadratureHelper<>::get(*triangulation, quad_refinements_or_intervals, false, additional_quad_refinements);
    ...
    MnFluxType mn_flux(grid_view,
                       *basis_functions,
                       ...
                       100);
    time = std::chrono::steady_clock::now() - begin;
    std::cout << "Creating entropy flux took: " << time.count() << " s!" << std::endl;
