...
{
  this->constructible_by_ctor(pdelab_rt_tolerance<TypeParam>());
}
TYPED_TEST(L2LocalProlongationOperatorTest, constructible_by_factory)
{
  this->constructible_by_factory(pdelab_rt_tolerance<TypeParam>());
}
TYPED_TEST(L2LocalProlongationOperatorTest, produces_correct_results)
{
  this->produces_correct_results(pdelab_rt_tolerance<TypeParam>());
}
