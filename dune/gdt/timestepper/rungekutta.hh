...
public:
  ...
  static Dune::DynamicVector<RangeFieldType> c()
  {
    return DSC::from_string<Dune::DynamicVector<RangeFieldType>>("[0 0.2 0.3 0.8 " + DSC::to_string(8.0 / 9.0, 15)
                                                                 + " 1 1]");
  }
...
template <class FluxOperatorImp, class SourceOperatorImp, class DiscreteFunctionImp, class TimeFieldImp,
          RungeKuttaMethods method = RungeKuttaMethods::euler, bool unsplit = false>
class RungeKutta
{
public:
   ...
   */
  RungeKutta(const FluxOperatorType& flux_operator, const SourceOperatorType& source_operator,
             const DiscreteFunctionType& initial_values, const DomainFieldType dx,
             const MatrixType A     = ButcherArrayProvider<RangeFieldType, method>::A(),
             const VectorType b     = ButcherArrayProvider<RangeFieldType, method>::b(),
             const TimeVectorType c = ButcherArrayProvider<RangeFieldType, method>::c())
  ...
  void solve(const TimeFieldType t_end, const TimeFieldType first_dt, const TimeFieldType save_step_length,
             const bool save_solution, const bool write_solution, const std::string filename_prefix,
  ...
  void solve(const TimeFieldType t_end, const TimeFieldType first_dt, const TimeFieldType save_step_length = 0.0,
             const bool save_solution = false, const bool write_solution = true, const std::string filename_prefix = "")
  {
