...
#ifndef DUNE_GDT_TIMESTEPPER_IMPLICIT_RUNGEKUTTA_PARALLEL_HH
#define DUNE_GDT_TIMESTEPPER_IMPLICIT_RUNGEKUTTA_PARALLEL_HH
...
class DiagonallyImplicitCodim0RungeKuttaTimeStepper : public TimeStepperInterface<DiscreteFunctionImp, TimeFieldImp>
{
...
public:
   ...
   */
  DiagonallyImplicitCodim0RungeKuttaTimeStepper(const OperatorType& op,
    ...
    , beta_(beta)
    , newton_matrices_(u_i_.space().grid_view().size(0),
                       MatrixType(u_i_.space().mapper().maxNumDofs(), u_i_.space().mapper().maxNumDofs(), 0.))
    , A_(A)
    , b_(b)
    , c_(c)
    , num_stages_(A_.rows())
  ...
  virtual TimeFieldType step(const TimeFieldType dt, const TimeFieldType max_dt) override final
  {
    ...
    for (size_t ii = 0; ii < num_stages_; ++ii) {
      ...
      if (XT::Common::FloatCmp::eq(A_[ii][ii], 0.0)) {
        ...
        const auto& grid_view = u_i_.space().grid_view().size(0);
        const auto& mapper = u_i_.space().mapper();
        const auto& index_set = grid_view.indexSet();

        //        std::vector<VectorType> entitywise_res(grid_view, VectorType(mapper.maxNumDofs(), mapper.maxNumDofs(),
        //        0.));
        ...
        //            entitywise_res[entity_index][ii] = res_.vector()[global_indices[ii]];
        //        }
        // Newton loop
        while (current_error > abs_tol && current_error > first_error * rel_reduction) {
          // assemble Newton matrix N = I - dt * a_{ii} * J_L
          op_.assemble_newton_matrix(
              newton_matrices_,
              u_i_,
              XT::Common::Parameter({{"t", {t + actual_dt * c_[ii]}}, {"factor", {-actual_dt * r_ * A_[ii][ii]}}}));
          // solve N d = res
          d_.vector() *= 0;
          op_.solve(newton_matrices_, res_, d_);
          ...
          while (current_error > error_threshold) {
            ...
            N_times_d_.vector() *= 0;
            op_.mv(newton_matrices_, d_, N_times_d_);
...
private:
  ...
  std::vector<MatrixType> newton_matrices_;
  const MatrixType A_;
  const VectorType b_;
  const VectorType c_;
...
#endif // DUNE_GDT_TIMESTEPPER_IMPLICIT_RUNGEKUTTA_PARALLEL_HH
