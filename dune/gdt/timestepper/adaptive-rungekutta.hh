...
enum class AdaptiveRungeKuttaMethods { bogacki_shampine, dormand_prince, other };
...
// unspecialized
template< class RangeFieldType, class TimeFieldType, AdaptiveRungeKuttaMethods method = AdaptiveRungeKuttaMethods::other >
struct AdaptiveButcherArrayProvider
{
  static Dune::DynamicMatrix< RangeFieldType > A()
  {
    DUNE_THROW(Dune::NotImplemented, "You have to provide a Butcher array in AdaptiveRungeKutta's constructor for this method!");
...
public:
  ...
  static Dune::DynamicVector< TimeFieldType > c()
  {
    return DSC::from_string< Dune::DynamicVector< TimeFieldType > >
 ...
 */
template< class OperatorImp, class DiscreteFunctionImp, class TimeFieldImp = double, AdaptiveRungeKuttaMethods method = AdaptiveRungeKuttaMethods::dormand_prince >
    ...
    : public TimeStepperInterface< DiscreteFunctionImp, TimeFieldImp >
{
  ...
  typedef typename internal::AdaptiveButcherArrayProvider< typename BaseType::RangeFieldType, TimeFieldImp, method > ButcherArrayProviderType;
public:
   ...
   */
  AdaptiveRungeKutta(const OperatorType& op,
  ...
  TimeFieldType step(const TimeFieldType dt, const TimeFieldType max_dt)
  {
    ...
    while (DSC::FloatCmp::gt(mixed_error, tol_)) {
      ...
      time_step_scale_factor = std::min(std::max(0.9*std::pow(tol_/mixed_error, 1.0/5.0), scale_factor_min_), scale_factor_max_);
