...
enum class RosenbrocktypeMethods { GRK4A, GRK4T, other };
...
struct RosenbrockButcherArrayProvider
{
  ...
  {
    DUNE_THROW(Dune::NotImplemented, "You have to provide a Butcher array in Rosenbrocktype's constructor for this method!");
...
struct RosenbrockButcherArrayProvider< RangeFieldType, TimeFieldType, RosenbrocktypeMethods::GRK4A >
{
  static Dune::DynamicMatrix< RangeFieldType > A()
  {
    return DSC::fromString< Dune::DynamicMatrix< RangeFieldType > >(std::string("[0 0 0 0;") +
                                                                    " 0.438 0 0 0;" +
  ...
  static Dune::DynamicMatrix< RangeFieldType > Gamma()
  {
    return DSC::fromString< Dune::DynamicMatrix< RangeFieldType > >(std::string("[0.395  0 0 0;") +
...
struct RosenbrockButcherArrayProvider< RangeFieldType, TimeFieldType, RosenbrocktypeMethods::GRK4T >
{
  ...
  static Dune::DynamicMatrix< RangeFieldType > Gamma()
  {
    return DSC::fromString< Dune::DynamicMatrix< RangeFieldType > >(std::string("[0.231 0 0 0;") +
 ...
 */
template< class OperatorImp, class DiscreteFunctionImp, class SolverImp, class TimeFieldImp = double, RosenbrocktypeMethods method = RosenbrocktypeMethods::GRK4T >
class Rosenbrocktype
    : public TimeStepperInterface< DiscreteFunctionImp, TimeFieldImp >
{
  ...
  typedef typename internal::RosenbrockButcherArrayProvider< typename BaseType::RangeFieldType, TimeFieldImp, method > ButcherArrayProviderType;
public:
   ...
   */
  Rosenbrocktype(const OperatorType& op,
  ...
  TimeFieldType step(const TimeFieldType dt, const TimeFieldType max_dt)
  {
    ...
    while (DSC::FloatCmp::gt(mixed_error, tol_)) {
      ...
      time_step_scale_factor = std::min(std::max(0.9*std::pow(tol_/mixed_error, 1.0/4.0), scale_factor_min_), scale_factor_max_);
