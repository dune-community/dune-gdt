...
public:
  ...
  size_t
  order(const LocalfunctionTupleType& /*localFunctions_in*/,
  ...
  void evaluate(
      const LocalfunctionTupleType& local_source_function,
      ...
      const Dune::FieldVector<DomainFieldType, dimDomain>& localPoint, Dune::DynamicVector<R>& ret) const
  {
    ...
    ret = Dune::DynamicVector<R>(
        std::get<0>(local_source_function)->evaluate(localPoint, entityAverage.evaluate(localPoint)[0]));
#else
    const auto fieldvector_ret =
        std::get<0>(local_source_function)->evaluate(localPoint, entityAverage.evaluate(localPoint)[0]);
