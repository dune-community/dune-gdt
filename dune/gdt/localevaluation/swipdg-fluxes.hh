#ifndef DUNE_DETAILED_DISCRETIZATIONS_EVALUATION_IPDG_FLUXES_HH
#define DUNE_DETAILED_DISCRETIZATIONS_EVALUATION_IPDG_FLUXES_HH
...
/**
 * \brief Traits for the SWIPDGFluxes::CouplingPrimal evaluation.
 */
template <class LocalizableFunctionImp>
...
template <class LocalizableFunctionImp>
class CouplingPrimal : public LocalEvaluation::Codim1Interface<CouplingPrimalTraits<LocalizableFunctionImp>, 4>
{
public:
                ...
                Dune::DynamicMatrix<R>& entityNeighborRet, Dune::DynamicMatrix<R>& neighborEntityRet) const
  {
    ...
    const R denominator = functionValueEn[0] + functionValueNe[0];
    const R penaltyParameter =
        (penaltyFactor_ / intersection.geometry().volume()) * ((functionValueEn[0] * functionValueNe[0]) / denominator);
    const R weightPlus  = functionValueNe[0] / denominator;
    const R weightMinus = functionValueEn[0] / denominator;
    // compute products
    ...
    for (size_t ii = 0; ii < rowsEn; ++ii) {
      ...
      for (size_t jj = 0; jj < colsEn; ++jj) {
        ...
        entityEntityRetRow[jj] += -weightPlus * functionValueEn * rangeTmp;
      ...
      for (size_t jj = 0; jj < colsNe; ++jj) {
        ...
        entityNeighborRetRow[jj] += weightPlus * functionValueEn * rangeTmp;
        normalTimesGradient = unitOuterNormal * ansatzGradientsNe[jj][0];
        rangeTmp            = testValuesEn[ii] * normalTimesGradient;
        entityNeighborRetRow[jj] += -weightMinus * functionValueNe * rangeTmp;
        evaluationTimesEvaluation = testValuesEn[ii] * ansatzValuesNe[jj];
        entityNeighborRetRow[jj] += -penaltyParameter * evaluationTimesEvaluation;
      } // loop over all neighbor ansatz basis functions
    } // loop over all entity test basis functions
    ...
    for (size_t ii = 0; ii < rowsNe; ++ii) {
      ...
      for (size_t jj = 0; jj < colsEn; ++jj) {
        rangeTmp = ansatzValuesEn[jj] * gradientTimesNormal;
        neighborEntityRetRow[jj] += -weightPlus * functionValueNe * rangeTmp;
        normalTimesGradient = unitOuterNormal * ansatzGradientsEn[jj][0];
        rangeTmp            = testValuesNe[ii] * normalTimesGradient;
        neighborEntityRetRow[jj] += weightPlus * functionValueNe * rangeTmp;
        rangeTmp = testValuesNe[ii] * ansatzValuesEn[jj];
        neighborEntityRetRow[jj] += -penaltyParameter * rangeTmp;
      } // loop over all entity ansatz basis functions
      // loop over all neighbor ansatz basis functions
      for (size_t jj = 0; jj < colsNe; ++jj) {
        rangeTmp = ansatzValuesNe[jj] * gradientTimesNormal;
        neighborNeighborRetRow[jj] += weightMinus * functionValueNe * rangeTmp;
        normalTimesGradient = unitOuterNormal * ansatzGradientsNe[jj][0];
        rangeTmp            = testValuesNe[ii] * normalTimesGradient;
        neighborNeighborRetRow[jj] += weightMinus * functionValueNe * rangeTmp;
...
public:
  ...
  static int order(const std::tuple<L>& localFunctions, const BaseFunctionSetInterface<T, D, d, R, rT, rCT>& testBase,
  ...
  void evaluate(const Dune::Stuff::LocalFunctionInterface<L, D, d, R, 1, 1>& localFunction,
                ...
                const BaseFunctionSetInterface<A, D, d, R, 1, 1>& ansatzBase, const IntersectionType& intersection,
                const Dune::FieldVector<D, d - 1>& localPoint, Dune::DynamicMatrix<R>& ret) const
  {
    typedef Dune::FieldVector<D, d> DomainType;
    ...
    const R penaltyParameter = (penaltyFactor_ / intersection.geometry().volume()) /** functionValue*/;
    // compute products
...
template <class LocalizableDiffusionFunctionImp, class LocalizableDirichletFunctionImp>
class BoundaryDirichletRHS
    : public LocalEvaluation::Codim1Interface<BoundaryDirichletRHSTraits<LocalizableDiffusionFunctionImp,
...
public:
  ...
  BoundaryDirichletRHS(const LocalizableDiffusionFunctionType& diffusion,
                       const LocalizableDirichletFunctionType& dirichlet, const double penaltyFactor)
  ...
  void evaluate(const std::tuple<LDF, LDR>& localFunctions, const BaseFunctionSetInterface<T, D, d, R, r, rC>& testBase,
                const IntersectionType& intersection, const Dune::FieldVector<D, d - 1>& localPoint,
                ...
                const Dune::FieldVector<D, d - 1>& localPoint, Dune::DynamicVector<R>& ret) const
  {
    ...
    testBase.jacobian(localPointEntity, testGradients);
    // evaluate penalty parameter
    const R penaltyParameter = (penaltyFactor_ / intersection.geometry().volume()) /** diffusionValue*/;
    // compute
...
#endif // DUNE_DETAILED_DISCRETIZATIONS_EVALUATION_IPDG_FLUXES_HH
