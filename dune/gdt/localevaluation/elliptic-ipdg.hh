...
class InnerTraits
  : public LocalEvaluation::internal::EllipticTraits< DiffusionFactorImp, DiffusionTensorImp >
{
public:
...
public:
  ...
  size_t order(const LocalfunctionTupleType& local_functions_en,
               ...
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_ne) const
  {
    ...
    return order(*local_diffusion_factor_en, *local_diffusion_tensor_en,
                 *local_diffusion_factor_ne, *local_diffusion_tensor_ne,
                 test_base_en, ansatz_base_en,
  ...
  void evaluate(const LocalfunctionTupleType& local_functions_en,
                const LocalfunctionTupleType& local_functions_ne,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_en,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_en,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_ne,
                ...
                Dune::DynamicMatrix< R >& ret_ne_en) const
  {
    ...
    const auto local_diffusion_factor_ne = std::get< 0 >(local_functions_ne);
    const auto local_diffusion_tensor_ne = std::get< 1 >(local_functions_ne);
    evaluate(*local_diffusion_factor_en, *local_diffusion_tensor_en,
             *local_diffusion_factor_ne, *local_diffusion_tensor_ne,
             test_base_en, ansatz_base_en,
             test_base_ne, ansatz_base_ne,
             intersection, local_point,
             ret_en_en,
             ret_ne_ne,
             ret_en_ne,
             ret_ne_en);
...
private:
  ...
  struct Helper
  {
    static_assert(AlwaysFalse< Anyone >::value,
                  "These variants of order and evaluate are only available for the single diffusion case (i.e., if DiffusionTensorImp is void)!");
  };
  ...
  {
    template< class R, size_t rT, size_t rCT, size_t rA, size_t rCA >
    static size_t order(const ThisType& ths,
                        ...
                        const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_ne)
    {
      ...
      const auto local_diffusion_tensor_en = std::get< 1 >(local_functions_en);
      const auto local_diffusion_tensor_ne = std::get< 1 >(local_functions_ne);
      return ths.order(local_diffusion_factor_en, *local_diffusion_tensor_en,
                   local_diffusion_factor_ne, *local_diffusion_tensor_ne,
                   test_base_en, ansatz_base_en,
                   test_base_ne, ansatz_base_ne);
    } // size_t order(...)
    ...
    static void evaluate(const ThisType& ths,
                         ...
                         Dune::DynamicMatrix< R >& ret_ne_en)
    {
      ...
      const auto local_diffusion_tensor_en = std::get< 1 >(local_functions_en);
      const auto local_diffusion_tensor_ne = std::get< 1 >(local_functions_ne);
      ths.evaluate(local_diffusion_factor_en, *local_diffusion_tensor_en,
                   local_diffusion_factor_ne, *local_diffusion_tensor_ne,
                   test_base_en, ansatz_base_en,
                   test_base_ne, ansatz_base_ne,
                   intersection, local_point,
                   ret_en_en, ret_ne_ne, ret_en_ne, ret_ne_en);
    }
  ...
  struct Helper< true, false, Anyone >
  {
    template< class R, size_t rT, size_t rCT, size_t rA, size_t rCA >
    static size_t order(const ThisType& ths,
                        const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, dimDomain, dimDomain >& local_diffusion_tensor_en,
                        const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, dimDomain, dimDomain >& local_diffusion_tensor_ne,
                        const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_en,
                        const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_en,
                        const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_ne,
                        const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_ne)
...
public:
  ...
  size_t order(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rD, rCD >& local_diffusion_en,
               ...
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_ne) const
  {
    return Helper< std::is_same< DiffusionTensorImp, void >::value, rD == 1 && rCD == 1 >
        ::order(*this,
                local_diffusion_en, local_diffusion_ne,
                test_base_en, ansatz_base_en,
                test_base_ne, ansatz_base_ne);
  } // ... order(...)

  template< class R, size_t rD, size_t rCD, size_t rT, size_t rCT, size_t rA, size_t rCA, class IntersectionType >
  void evaluate(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rD, rCD >& local_diffusion_en,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rD, rCD >& local_diffusion_ne,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_en,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_en,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_ne,
                ...
                Dune::DynamicMatrix< R >& ret_ne_en) const
  {
    Helper< std::is_same< DiffusionTensorImp, void >::value, rD == 1 && rCD == 1 >
        ::evaluate(*this,
                   local_diffusion_en, local_diffusion_ne,
                   test_base_en, ansatz_base_en,
                   test_base_ne, ansatz_base_ne,
  ...
  size_t order(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rDF, rCDF >& local_diffusion_factor_en,
               const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rDT, rCDT >& local_diffusion_tensor_en,
               const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rDF, rCDF >& local_diffusion_factor_ne,
               const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rDT, rCDT >& local_diffusion_tensor_ne,
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_en,
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_en,
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base_ne,
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base_ne) const
  ...
  template< class R, class IntersectionType >
  void evaluate(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& local_diffusion_factor_en,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, dimDomain, dimDomain >& local_diffusion_tensor_en,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& local_diffusion_factor_ne,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, dimDomain, dimDomain >& local_diffusion_tensor_ne,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& test_base_en,
...
//    const auto local_diffusion_factor = (local_diffusion_factor_en + local_diffusion_factor_ne) * 0.5;
    ...
    const size_t max_polorder = std::max(test_base_en.order(),
                                         std::max(ansatz_base_en.order(),
                                                  std::max(test_base_ne.order(),
                                                           ansatz_base_ne.order())));
...
template< class DiffusionFactorImp, class DiffusionTensorImp, Method method >
class BoundaryLHS
  : public LocalEvaluation::Codim1Interface< internal::BoundaryLHSTraits< DiffusionFactorImp, DiffusionTensorImp, method >,
                                             2 >
{
public:
  typedef Elliptic< DiffusionFactorImp, DiffusionTensorImp >            EllipticType;
...
private:
  ...
  struct Helper< true, true, Anyone >
  {
                         ...
                         Dune::DynamicMatrix< R >& ret)
    {
      const auto local_functions = ths.localFunctions(local_diffusion_factor.entity());
      const auto local_diffusion_tensor = std::get< 1 >(local_functions);
      ths.evaluate(local_diffusion_factor, *local_diffusion_tensor,
                   test_base, ansatz_base,
                   intersection, local_point,
                   ret);
    }
  }; // struct Helper< true, true, ... >
...
public:
  ...
  template< class R, size_t rD, size_t rCD, size_t rT, size_t rCT, size_t rA, size_t rCA >
  size_t order(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rD, rCD >& local_diffusion,
               const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base,
  ...
  template< class R, size_t rD, size_t rCD, size_t rT, size_t rCT, size_t rA, size_t rCA, class IntersectionType >
  void evaluate(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rD, rCD >& local_diffusion,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rT, rCT >& test_base,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, rA, rCA >& ansatz_base,
                const IntersectionType& intersection,
                const Dune::FieldVector< DomainFieldType, dimDomain - 1 >& local_point,
  ...
  template< class R, size_t rDF, size_t rCDF, size_t rDT, size_t rCDT, size_t rT, size_t rCT, size_t rA, size_t rCA >
  size_t order(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rDF, rCDF >& local_diffusion_factor,
  ...
  template< class R, class IntersectionType >
  void evaluate(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& local_diffusion_factor,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, dimDomain, dimDomain >& local_diffusion_tensor,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& test_base,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& ansatz_base,
                const IntersectionType& intersection,
...
class BoundaryRHS
  : public LocalEvaluation::Codim1Interface
        < internal::BoundaryRHSTraits< DirichletImp, DiffusionFactorImp, DiffusionTensorImp, method >, 1 >
{
  typedef Elliptic< DiffusionFactorImp, DiffusionTensorImp >                          EllipticType;
...
public:
                ...
                Dune::DynamicVector< R >& ret) const
  {
    ...
    const auto local_diffusion_factor = std::get< 1 >(local_functions);
    const auto local_diffusion_tensor = std::get< 2 >(local_functions);
    evaluate(*local_dirichlet, *local_diffusion_factor, *local_diffusion_tensor, test_base, intersection, local_point, ret);
  }
  ...
  size_t order(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, rLR, rCLR >& local_dirichlet,
  ...
  template< class R, class IntersectionType >
  void evaluate(const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& local_dirichlet,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& local_diffusion_factor,
                const Stuff::LocalfunctionInterface< EntityType, DomainFieldType, dimDomain, R, dimDomain, dimDomain >& local_diffusion_tensor,
                const Stuff::LocalfunctionSetInterface< EntityType, DomainFieldType, dimDomain, R, 1, 1 >& test_base,
                const IntersectionType& intersection,
