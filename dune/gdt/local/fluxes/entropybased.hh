...
public:
  ...
  virtual FluxRangeType evaluate(const RangeType& u, const E& entity, const DomainType& x_local, const double t) const
  {
    ...
    for (size_t ll = 0; ll < quadrature_.size(); ++ll) {
      ...
      for (size_t dd = 0; dd < dimDomain; ++dd) {
        auto m = M_[ll];
        m *= position[dd] * std::exp(alpha * m) * weight;
        for (size_t rr = 0; rr < u.size(); ++rr)
          ret[rr][dd] += m[rr];
      }
    }
...
{
  typedef AnalyticalFluxInterface<E, D, d, R, rangeDim, rC> BaseType;
  typedef EntropyBasedLocalFlux3D<GridViewType, E, D, d, R, rangeDim, rC, num_quad_points> ThisType;
...
public:
  ...
  RangeType get_alpha(const RangeType& u, const E& entity, const DomainType& x_local, const double t) const
  {
    ...
    if ((*alpha_cache_)[index] && XT::Common::FloatCmp::eq((*alpha_cache_)[index]->first, t)) {
      ...
      alpha_iso *= std::log(psi_iso);
      auto u_iso =
          Dune::GDT::Hyperbolic::Problems::PointSourcePnHatFunctions<E, D, d, R, dimRange>::basisfunctions_integrated(
              quadrature_, poly_);
      u_iso *= psi_iso;
                                            ...
                                            const double t) const
  {
    ...
    for (size_t ll = 0; ll < quadrature_.size(); ++ll) {
      ...
      for (size_t dd = 0; dd < dimDomain; ++dd) {
        if (position * n_ij > 0)
          m *= position[dd] * std::exp(alpha_i * m) * weight * n_ij[dd];
        else
          m *= position[dd] * std::exp(alpha_j * m) * weight * n_ij[dd];
        ret += m;
      } // dd
    }
    return ret;
