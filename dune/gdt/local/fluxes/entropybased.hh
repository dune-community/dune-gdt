...
public:
  ...
  explicit EntropyBasedLocalFlux(
    ...
    , regularization_parameters_(index_set_.size(0), 0.)
    , regularization_times_(index_set_.size(0), std::numeric_limits<RangeFieldType>::lowest())
  {
  ...
  public:
    ...
    Localfunction(const EntityType& e,
                  ...
                  std::mutex& mutex,
                  RangeFieldType& regularization_parameter,
                  RangeFieldType& regularization_time)
      : LocalfunctionType(e)
      , basis_functions_(basis_functions)
      , quadrature_(quadrature)
      , M_(M)
      ...
      , regularization_time_(regularization_time)
    {
    }
    ...
    {
      get_alpha(x_local, u, param);
    }

    VectorType get_alpha(const DomainType& /*x_local*/,
                         const StateRangeType& u,
                         ...
                         const bool regularize = true) const
    {
      ...
      mutex_.lock();
      if (XT::Common::FloatCmp::ne(regularization_time_, t) && !boundary) {
      ...
      if (cache_iterator != cache_.end() && XT::Common::FloatCmp::eq(cache_iterator->first, u)) {
        ...
        for (const auto& r : r_sequence) {
          regularization_parameter_ = std::max(regularization_parameter_, r);
          // regularize u
  ...
  private:
    ...
    std::mutex& mutex_;
    RangeFieldType& regularization_parameter_;
    RangeFieldType& regularization_time_;
  }; // class Localfunction
  ...
  std::unique_ptr<Localfunction> derived_local_function(const EntityType& entity) const
  {
    ...
    return std::make_unique<Localfunction>(entity,
                                           ...
                                           cache_[index],
                                           mutexes_[index],
                                           regularization_parameters_[index],
                                           regularization_times_[index]);
  }
  ...
  {
    std::fill(regularization_parameters_.begin(), regularization_parameters_.end(), 0.);
  }

  const std::vector<RangeFieldType>& regularization_parameters() const
  {
    return regularization_parameters_;
  }
...
private:
  ...
  mutable std::vector<RangeFieldType> regularization_parameters_;
  mutable std::vector<RangeFieldType> regularization_times_;
};
#endif
...
public:
  ...
  using BaseType::dimRange;
  using BaseType::dimRangeCols;
  static const size_t dimQuadrature = dimDomain;
  ...
  public:
    ...
    virtual void calculate_alpha(const DomainType& x_local,
                                 ...
                                 const XT::Common::Parameter& param) const override
    {
      get_alpha(x_local, u_in, param);
    }

    virtual VectorType get_alpha(const DomainType& /*x_local*/,
                                 const StateRangeType& u_in,
                                 const XT::Common::Parameter& param,
                                 const bool regularize = true) const
    {
          ...
          *T_k = T_minus_one_;
          ...
          for (size_t kk = 0; kk < k_max_; ++kk) {
            ...
            if (two_norm(error) < tau_ && std::exp(5 * one_norm(T_k_inv_transp_d_k)) < 1 + epsilon_gamma_) {
              for (size_t jj = 0; jj < num_blocks; ++jj)
                XT::LA::solve_lower_triangular_transposed((*T_k)[jj], alpha[jj], beta_out[jj]);
              for (size_t jj = 0; jj < num_blocks; ++jj)
                for (size_t ii = 0; ii < block_size; ++ii)
  ...
  virtual const std::vector<RangeFieldType>& regularization_parameters() const override
  {
    return regularization_parameters_;
  }
...
public:
  explicit EntropyBasedLocalFlux(
    ...
    , mutexes_(index_set_.size(0))
    , regularization_parameters_(index_set_.size(0))
    , regularization_times_(index_set_.size(0))
  {
  ...
  public:
    ...
    StateRangeType get_alpha(const DomainType& /*x_local*/,
                             const StateRangeType& u,
                             const XT::Common::Parameter& param,
                             const bool regularize = true) const
    {
      const bool boundary = bool(param.get("boundary")[0]);
      StateRangeType alpha(0.);
      const RangeFieldType t = param.get("t")[0];
...
public:
  ...
  typedef EntropyLocalCache<StateRangeType, RangeType> LocalCacheType;
  static const size_t cache_size = 2 * dimRange + 2;
  ...
  public:
    ...
    RangeType get_alpha(const DomainType& /*x_local*/,
                        const StateRangeType& u,
                        const XT::Common::Parameter& param,
                        const bool regularize = true) const
    {
