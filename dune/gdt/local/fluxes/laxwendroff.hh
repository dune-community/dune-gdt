...
public:
                     ...
                     const RangeType& u_j) const
  {
    ...
    auto f_u_i = XT::Functions::RangeTypeConverter<dimRange, dimDomain>::convert(
        local_flux_inside->evaluate(x_in_inside_coords, u_i, param_inside_));
    auto f_u_j = XT::Functions::RangeTypeConverter<dimRange, dimDomain>::convert(
    ...
    ret += u_j;
    ret *= 0.5;
    RangeType second_part;
    for (size_t kk = 0; kk < dimRange; ++kk)
      second_part[kk] = f_u_j[kk][direction] - f_u_i[kk][direction];
    second_part *= n_ij[direction] * 0.5 * dt_ / dx * dimDomain;
    ret -= second_part;
    auto flux_eval = local_flux_inside->evaluate(x_in_inside_coords, ret, param_inside_);
    for (size_t kk = 0; kk < dimRange; ++kk)
      ret[kk] = flux_eval[kk][direction] * n_ij[direction];
    return ret;
  } // ... evaluate(...)
