...
public:
  ...
  using FluxEigenDecompositionLambdaType =
      std::function<std::tuple<std::vector<XT::Common::real_t<R>>,
                               ...
                               XT::Common::FieldMatrix<XT::Common::real_t<R>, m, m>>(
          const FieldVector<R, m>&, const FieldVector<double, d>&, const XT::Common::Parameter& param)>;

  NumericalVijayasundaramFlux(const FluxType& flx)
    : BaseType(flx)
    , flux_eigen_decomposition_([&](const auto& w, const auto& n, const auto& param) {
      // evaluate flux jacobian, compute P matrix [DF2016, p. 404, (8.17)]
      const auto df = this->flux().jacobian(w, param);
      const auto P = df * n;
                       ...
                       const XT::Common::Parameter& param = {}) const override final
  {
    ...
    const auto eigendecomposition = flux_eigen_decomposition_(0.5 * (u + v), n, param);
