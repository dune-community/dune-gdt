...
template< class QuaternaryFaceIntegrandTypeType >
class LocalCouplingIntegralOperator
  : public LocalCouplingTwoFormInterface< internal::LocalCouplingIntegralOperatorTraits< QuaternaryFaceIntegrandTypeType > >
{
public:
              ...
              Dune::DynamicMatrix< R >& ret_ne_en) const
  {
    ...
    // quadrature
    const size_t integrand_order = integrand_.order(local_functions_en, local_functions_ne,
                                                     test_base_en, ansatz_base_en,
                                                     test_base_ne, ansatz_base_ne) + over_integrate_;
    const auto& quadrature = QuadratureRules< D, d - 1 >::rule(intersection.type(),
    ...
    Dune::DynamicMatrix< R > evaluation_result_en_en(rows_en, cols_en, 0.);  // \todo: make mutable member, after SMP refactor
    Dune::DynamicMatrix< R > evaluation_result_ne_ne(rows_ne, cols_ne, 0.);  // \todo: make mutable member, after SMP refactor
    Dune::DynamicMatrix< R > evaluation_result_en_ne(rows_en, cols_ne, 0.);  // \todo: make mutable member, after SMP refactor
    Dune::DynamicMatrix< R > evaluation_result_ne_en(rows_ne, cols_en, 0.);  // \todo: make mutable member, after SMP refactor
    // loop over all quadrature points
    for (const auto& quadrature_point : quadrature) {
      const auto xx = quadrature_point.position();
      ...
      const auto quadrature_weight = quadrature_point.weight();
      // evaluate local
      integrand_.evaluate(local_functions_en, local_functions_ne,
                           test_base_en, ansatz_base_en,
