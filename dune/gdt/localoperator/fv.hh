    ...
    for (auto i_it = grid_view.ibegin(entity); i_it != i_it_end; ++i_it) {
      ...
      if (intersection.neighbor()) {
        ...
        if ((neighbor_center[0] < entity_center[0] && !boundary) || (neighbor_center[0] > entity_center[0] && boundary))
          u_left = source.local_discrete_function(*neighbor_ptr)->evaluate(neighbor_ptr->geometry().local(neighbor_center));
        else
          u_right = source.local_discrete_function(*neighbor_ptr)->evaluate(neighbor_ptr->geometry().local(neighbor_center));
      } else {
