...
public:
  ...
  std::set<size_t> local_dirichlet_DoFs_order_1(const EntityType& entity, const BoundaryInfoType& boundaryInfo) const
  {
    ...
    for (const auto& global_vertex : global_dirichlet_vertices) {
      ...
      if (reference_element.checkInside(local_vertex)
          && std::find(dirichlet_vertices.begin(), dirichlet_vertices.end(), local_vertex) == dirichlet_vertices.end())
...
private:
  void add_dirichlet_vertices(const BoundaryInfoType& boundaryInfo, const EntityType& entity,
                              const ssize_t recursion_level, std::vector<DomainType>& dirichlet_vertices) const
  {
