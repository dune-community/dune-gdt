...
public:
  ...
  std::set< size_t > local_dirichlet_DoFs_simplicial_lagrange_elements(const EntityType& entity,
                                                                       const BoundaryInfoType& boundaryInfo) const
  {
    if (!entity.type().isSimplex()) DUNE_THROW(NotImplemented, "Only implemented for simplex elements!");
    // check
    assert(this->grid_view().indexSet().contains(entity));
    // prepare
    std::set< size_t > localDirichletDofs;
    ...
    for (auto intersection_it = this->grid_view().ibegin(entity);
      ...
      possible_convex_combination_coefficients(possible_coefficient_vectors,
                                               possible_coefficients,
                                               dirichlet_vertices_intersection.size());
                                                ...
                                                const size_t final_size) const
  {
    if (vectors_in.empty())
    {
      ...
      for (auto& vec : vectors_in) {
        for (const auto& coeff : possible_coefficients) {
          if ((vec.size() != final_size - 1 && DSC::FloatCmp::le(std::accumulate(vec.begin(), vec.end(), 0.0) + coeff, 1.0))
              || (vec.size() == final_size -1 && DSC::FloatCmp::eq(std::accumulate(vec.begin(), vec.end(), 0.0) + coeff, 1.0))) {
