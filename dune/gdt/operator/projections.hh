...
class DirichletProjection
{
  ...
  static_assert(std::is_same< RangeImp,
                              DiscreteFunctionDefault<  ContinuousLagrangeSpace::FemWrapper<  typename RangeImp::SpaceType::GridPartType,
                                                                                              1,
                                                                                              typename RangeImp::SpaceType::RangeFieldType,
                                                                                              1 >,
  ...
  void apply(const SourceType& source, RangeType& range) const
  {
    ...
    for (auto entity_it = range.space().gridPart().template begin< 0 >(); entity_it != entity_it_end; ++entity_it) {
      ...
      if (entity.hasBoundaryIntersections()) {
        ...
        for (auto intersection_it = range.space().gridPart().ibegin(entity);
