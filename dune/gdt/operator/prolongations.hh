...
public:
  ...
  static void
  apply(const DiscreteFunctionDefaultConst<ContinuousLagrangeSpace::FemWrapper<SGP, sp, R, dimRange>, SV>& source,
  ...
  template <class SGP, int sp, class R, int dimRange, class SV, class RGP, int rp, class RV>
  static void
  apply(const DiscreteFunctionDefaultConst<DiscontinuousLagrangeSpace::FemLocalfunctionsWrapper<SGP, sp, R, dimRange>,
                                           SV>& source,
        DiscreteFunctionDefault<DiscontinuousLagrangeSpace::FemLocalfunctionsWrapper<RGP, rp, R, dimRange>, RV>& range)
  {
    ...
    // create search in the source grid part
    typedef DiscreteFunctionDefaultConst<DiscontinuousLagrangeSpace::FemLocalfunctionsWrapper<SGP, sp, R, dimRange>, SV>
        SourceType;
    typedef DiscreteFunctionDefault<DiscontinuousLagrangeSpace::FemLocalfunctionsWrapper<RGP, rp, R, dimRange>, RV>
        RangeType;
      ...
      for (const auto& quadrature_point : quadrature) {
        ...
        for (size_t ii = 0; ii < local_basis.size(); ++ii) {
          ...
          for (size_t jj = 0; jj < local_basis.size(); ++jj) {
            local_matrix[ii][jj] += integration_element * quadrature_weight * (basis_values[ii] * basis_values[jj]);
          }
        }
        ++pp;
      } // loop over all quadrature points
