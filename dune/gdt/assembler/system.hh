...
public:
  ...
  void addLocalAssembler(const LocalAssembler::Codim1CouplingMatrix<L>& localAssembler,
                         const AssembleOnFunctorType functor, Dune::Stuff::LA::MatrixInterface<M>& matrix)
  {
...
private:
  ...
  class LocalMatrixConstraintsApplicationWrapper : public LocalConstraintsApplication
  {
  public:
    LocalMatrixConstraintsApplicationWrapper(ConstraintsType& constraints, Dune::Stuff::LA::MatrixInterface<M>& matrix)
  ...
  class LocalVectorConstraintsApplicationWrapper : public LocalConstraintsApplication
  {
  public:
    LocalVectorConstraintsApplicationWrapper(ConstraintsType& constraints, Dune::Stuff::LA::VectorInterface<V>& vector)
      : constraints_(constraints)
      , vector_(vector)
    {
    }
...
public:
  template <class ConstraintsType, class M>
  void addLocalConstraints(ConstraintsType& constraints, Dune::Stuff::LA::MatrixInterface<M>& matrix)
  {
    assert(matrix.rows() == testSpace_.mapper().size());
    assert(matrix.cols() == ansatzSpace_.mapper().size());
  ...
  template <class ConstraintsType, class V>
  void addLocalConstraints(ConstraintsType& constraints, Dune::Stuff::LA::VectorInterface<V>& vector)
  {
    assert(vector.size() == testSpace_.mapper().size());
