...
public:
  ...
  Vectors(const std::vector<size_t>& num_tmp_objects, const size_t max_size)
    : vectors_({std::vector<LocalVectorType>(safely_get(num_tmp_objects, 0), LocalVectorType(max_size, FieldType(0))),
                std::vector<LocalVectorType>(safely_get(num_tmp_objects, 1), LocalVectorType(max_size, FieldType(0)))})
    , indices_(max_size)
  {
  }
