...
/**
 *
 * This code realizes extented dirichlet constraint. It declares the inner dofs that share intersection dofs
 * with the boundary.
 */

template <class IntersectionType, class SpaceType>
class ExtendedDirichletConstraints
  : public Dune::XT::Grid::ElementFunctor<typename SpaceType::GridViewType>
  , public XT::Common::ThreadResultPropagator<ExtendedDirichletConstraints<IntersectionType, SpaceType>,
                                              std::set<size_t>,
                                              internal::setUnion<size_t>>
{
  using ThisType = ExtendedDirichletConstraints<IntersectionType, SpaceType>;
  using BaseType = XT::Grid::ElementFunctor<typename SpaceType::GridViewType>;
  using Propagator = XT::Common::ThreadResultPropagator<ThisType, std::set<size_t>, internal::setUnion<size_t>>;
  friend Propagator;
...
public:
  ...
  ExtendedDirichletConstraints(const BoundaryInfoType& bnd_info, const SpaceType& space)
    : BaseType()
    , Propagator(this)
    , boundary_info_(bnd_info)
    , space_(space)
  ...
  ExtendedDirichletConstraints(const ThisType& other)
    : BaseType(other)
    , Propagator(this)
    , boundary_info_(other.boundary_info_)
    , space_(other.space_)
...
template <class GV, size_t r, size_t rC, class R>
ExtendedDirichletConstraints<XT::Grid::extract_intersection_t<GV>, SpaceInterface<GV, r, rC, R>>
make_dirichlet_constraints(const SpaceInterface<GV, r, rC, R>& space,
                           const XT::Grid::BoundaryInfo<XT::Grid::extract_intersection_t<GV>>& boundary_info)
{
  return ExtendedDirichletConstraints<XT::Grid::extract_intersection_t<GV>, SpaceInterface<GV, r, rC, R>>(boundary_info,
                                                                                                          space);
}
