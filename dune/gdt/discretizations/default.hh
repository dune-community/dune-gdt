...
class NonStationaryDefaultTraits
{
  // no checks of the arguments needed, those are done in the interfaces
public:
  typedef NonStationaryDefault<ProblemImp, FVSpaceImp> derived_type;
  typedef ProblemImp ProblemType;
...
{
  typedef NonStationaryDiscretizationInterface<internal::NonStationaryDefaultTraits<ProblemImp, FVSpaceImp>> BaseType;
...
public:
  ...
  void solve(DiscreteSolutionType& solution, const bool is_linear) const
  {
    ...
    try {
      ...
      typedef typename Dune::GDT::TimeStepper::ExplicitRungeKutta<OperatorType, FVFunctionType, double>
          OperatorTimeStepperType;
      OperatorTimeStepperType timestepper_op(advection_operator, u, -1.0);
      typedef typename Dune::GDT::TimeStepper::ExplicitRungeKutta<RHSOperatorType, FVFunctionType, double>
          RHSOperatorTimeStepperType;
      RHSOperatorTimeStepperType timestepper_rhs(rhs_operator, u);
      typedef typename Dune::GDT::TimeStepper::FractionalStepStepper<OperatorTimeStepperType,
                                                                     RHSOperatorTimeStepperType> TimeStepperType;
