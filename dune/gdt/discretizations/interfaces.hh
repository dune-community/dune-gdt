...
public:
  ...
  Stuff::Common::Configuration solver_options(const std::string type = "") const
  {
    ...
    if (opts.empty())
      DUNE_THROW(Stuff::Exceptions::internal_error,
                 "Reported solver_options() of the derived class (see below) for type '" << type
                 << "'must not be empty!\n\n  " << Stuff::Common::Typename< derived_type >::value());
    return opts;
  } // ... solver_options(...)
...
struct is_stationary_discretization
  : public std::is_base_of< StationaryDiscretizationInterface< typename D::Traits >, D >
{};


template< class D >
struct is_stationary_discretization< D, false >
