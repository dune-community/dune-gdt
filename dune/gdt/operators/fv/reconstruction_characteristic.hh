...
public:
  ...
  void apply_local(const EntityType& entity)
  {
    ...
    for (size_t dd = 0; dd < dimDomain; ++dd)
      helper<dimDomain>::reconstruct(dd, values, quadrature_, reconstructed_values_map, intersections);
  } // void apply_local(...)
...
private:
  ...
  struct helper<1, anything>
  {
                            ...
                            const IntersectionVectorType& intersections)
    {
      ...
      static auto quadrature_point = FieldVector<DomainFieldType, dimDomain - 1>();
      for (size_t ii = 0; ii < 2; ++ii)
        reconstructed_values_map.insert(
            std::make_pair(intersections[ii].geometryInInside().global(quadrature_point), reconstructed_values[ii]));
                            ...
                            const IntersectionVectorType& intersections)
    {
      ...
      for (size_t ii = 0; ii < stencil_size; ++ii)
        for (size_t jj = 0; jj < stencil_size; ++jj)
          if (dd == 0)
            char_values[jj][ii] = values[ii][jj][0];
          else if (dd == 1)
            char_values[ii][jj] = values[ii][jj][0];
      ...
      for (size_t ii = 0; ii < 2; ++ii) {
        for (size_t jj = 0; jj < num_quad_points; ++jj) {
          ...
          reconstructed_values_map.insert(std::make_pair(
              intersections[2 * dd + ii].geometryInInside().global(quadrature_point), reconstructed_values[ii][jj]));
        } // jj
      } // ii
    } // static void reconstruct(...)
      ...
      for (size_t ii = 0; ii < stencil_size; ++ii) {
        for (size_t jj = 0; jj < stencil_size; ++jj) {
          for (size_t kk = 0; kk < stencil_size; ++kk) {
            if (dd == 0)
              char_values[kk][jj][ii] = values[ii][jj][kk];
            else if (dd == 1)
              char_values[ii][kk][jj] = values[ii][jj][kk];
            else if (dd == 2)
              char_values[jj][ii][kk] = values[ii][jj][kk];
          }
        }
      }
      ...
      for (size_t ii = 0; ii < 2; ++ii) {
        for (size_t jj = 0; jj < num_quad_points; ++jj) {
          for (size_t kk = 0; kk < num_quad_points; ++kk) {
            ...
            reconstructed_values_map.insert(
                std::make_pair(intersections[2 * dd + ii].geometryInInside().global(quadrature_point),
                               reconstructed_values[ii][jj][kk]));
          } // kk
        } // jj
      } // ii
    } // static void reconstruct(...)
