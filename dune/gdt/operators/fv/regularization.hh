...
template <class SourceType, class RangeType, class AnalyticalFluxType>
...
public:
  ...
  void apply_local(const EntityType& entity)
  {
    ...
    const auto s = analytical_flux_.local_function(entity)->calculate_alpha(x_in_inside_coords, u, param_);
    ...
    if (s > 0.) {
      ...
      const auto u_iso = analytical_flux_.basis_functions().calculate_isotropic_distribution(u).first;
      for (size_t ii = 0; ii < vector_indices.size(); ++ii) {
        const auto& vec_index = vector_indices[ii];
        range_.vector().set_entry(vec_index, (1 - s) * source_.vector().get_entry(vec_index) + s * u_iso[ii]);
      }
    }
  } // void apply_local(...)
...
template <class AnalyticalFluxType, class Traits>
...
struct MomentRegularizerTraits
{
  ...
  using derived_type = MomentRegularizer<AnalyticalFluxType, MomentRegularizerTraits>;
};
...
template <class AnalyticalFluxImp, class Traits = internal::MomentRegularizerTraits<AnalyticalFluxImp>>
...
public:
  ...
  {
    LocalMomentRegularizer<SourceType, RangeType, AnalyticalFluxType> local_moment_regularizer(
