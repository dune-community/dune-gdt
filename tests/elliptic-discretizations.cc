  ...
  : public ::testing::Test
{
  void check() const
  {
    ...
    auto errors = eoc_study.run(true, out);
  ...
  : public ::testing::Test
{
  void check() const
  {
    if (std::is_same< TestCase, EllipticTestCase::Spe10Model1< Dune::ALUConformGrid< 2, 2 > > >::value) {
      std::cerr << "EllipticSIPDGDiscretization does not work for EllipticTestCase::Spe10Model1< Dune::ALUConformGrid< 2, 2 > >!";
    } else {
      const TestCase test_case;
      test_case.print_header(out);
      out << std::endl;
      ...
      auto errors_1 = eoc_study_1.run(true, out);
      out << std::endl;
      EllipticSIPDG::EocStudy< TestCase, 2 > eoc_study_2(test_case);
      auto errors_2 = eoc_study_2.run(true, out);
